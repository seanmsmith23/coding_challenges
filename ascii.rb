...
Irns}$xlmw$jyr2$Xli$xvyxl${mpp$fi$liph$f}$xlswi${ls$ger$higshi$xli$qiwweki2$>-".each_char.map(&:ord).to_a

array_new = array.map { |x| x - 4 }
back_to_char = array_new.map { |x| x.chr }
decoded = back_to_char.join""
puts decoded
